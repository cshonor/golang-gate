...
```go
运行
func f() (result int) { // 具名返回值
    defer func() {
        result++ // defer 在 return 后执行，直接修改返回值
    }()
    return 1 // 第一步：赋值 result = 1
             // 第二步：执行 defer → result=2
             // 第三步：RET 返回 2
}
...
func f() int { // 匿名返回值
    result := 1
    defer func() {
        result++ // 改的是局部变量，不是返回值
    }()
    return result // 第一步：把 result=1 拷贝到“临时返回寄存器”
                  // 第二步：执行 defer → result=2（没用）
                  // 第三步：RET 返回寄存器里的 1
}
