...
具名返回值（Named Return Values），也称命名返回值，是 Go 语言函数的核心特性：在函数签名的返回值部分同时声明类型与变量名
```go
// 匿名返回值（传统写法）
func funcName(参数) (类型1, 类型2) {
    // 必须显式 return 值
    return v1, v2
}

// 具名返回值（命名写法）
func funcName(参数) (名1 类型1, 名2 类型2) {
    // 名1、名2 等效函数内局部变量
    名1 = ...
    名2 = ...
    return       // 裸返回：自动返回名1、名2当前值
    // 或显式 return 名1, 名2
}
```
...
1. **具名返回值（named return）**：`defer` 能改返回值。

```go
func f() (result int) {
	defer func() {
		result++ // return 后置步骤里改具名返回值
	}()
	return 1
}
...
2. **匿名返回值**：`defer` 改的是局部变量，**改不到**已经参与 `return` 的那份「返回槽」拷贝（教学模型下可理解为：先拷贝再 defer）。
...
- **具名返回值**：栈上已有「返回值变量」，`return` 先往它赋值，`defer` 还能继续改它。
- **匿名返回值**：`return expr` 先把值拷贝到返回路径，`defer` 里改局部变量不影响已确定的返回值（细节以实现为准，这里记工程直觉即可）。
