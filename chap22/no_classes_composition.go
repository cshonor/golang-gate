在 Go 里，结构体的构造函数必须手动创建，Go 不会自动生成。而且构造函数是普通函数，不是“构造方法”——它没有结构体接收者，只是返回结构体指针的函数。
 
比如你定义  Product  结构体后，Go 不会自动给你一个构造函数，需要你自己写：
...
调用时直接  p := NewProduct("1", "商品1") ，和调用  fmt.Println  这种普通函数一样。
 
而“结构体方法”是有接收者的函数，比如给  Product  加一个方法：
 
go  
// 结构体方法，有接收者 p Product
func (p Product) GetName() string {
    return p.Name
}
 
 
方法必须通过结构体实例调用，比如  p.GetName() ，而构造函数是独立的函数，直接调用就能创建实例。
 
简单说：构造函数是“创建结构体的普通函数”，方法是“属于结构体的函数”，两者完全不同，构造函数需要你手动写，Go 不会自动生成。你之前的  server  结构体，如果想让创建更规范，就需要手动写  NewServer  这样的构造函数。// 示例：Go 语言没有类，通过结构+方法+组合实现面向对象
