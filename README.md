...
9. [并发模式与同步原语](./09-concurrency_patterns/README.md) — 在 GMP/锁之上收束常见写法与反模式。  
10. [Context（中间件的灵魂）](./10-context_pkg/README.md) — 超时、取消、请求级数据；**写 HTTP/RPC 前建议完成**。  
11. [GC 与内存管理](./11-GC%20and%20memory/README.md) — 逃逸、分配、调优；有并发与 IO 直觉后读更落地。  
12. [Go 网络编程 + IO 模型](./12-go-network/README.md) — 综合 OS、TCP/UDP、`net`/`netpoll`；**依赖 7～10 的并发与 Context 基础**。  
13. [unsafe（unsafe 包）](./13-unsafe_pkg/README.md) — 放最后：在已熟悉类型系统、内存与并发边界后再碰。
