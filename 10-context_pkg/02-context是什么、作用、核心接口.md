...
说明：`WithCancel` / `WithTimeout` 都会返回 **`CancelFunc`**，**在 goroutine 或函数结束前调用 `cancel()`** 可尽早释放子树资源（避免泄漏）。`WithDeadline` 见 [05-WithTimeout 超时控制.md](./05-WithTimeout%20超时控制.md)。
...
| 中间件 | [07-context在中间件中的实战.md](./07-context在中间件中的实战.md) |
